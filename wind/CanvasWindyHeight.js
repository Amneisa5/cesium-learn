...
export default class CanvasWindyHeight {
  constructor(json, params) {
    // Generate mock height data if not provided
    json = this._addMockHeightData(json);
  ...
  _addMockHeightData (json) {
    // Add mock height/wind data (wdata)
    if (!json.wdata) {
      json.wdata = [];
  ...
  _init () {
    ...
    for (var i = 0; i < this._particlesNumber; i++) {
      this._particles.push(this._randomParticle(new CanvasHeightParticle()));
    }
    ...
    this._particles.forEach((particle) => {
      ...
      if (particle.age > 0) {
        ...
        if (!this._isInExtent(tlng, tlat)) {
          ...
          nextLng = tlng + this.calc_speedRate[0] * uvw[0]
          nextLat = tlat + this.calc_speedRate[1] * uvw[1]
          particle.lng = tlng
          particle.lat = tlat
          particle.x = tx
          particle.y = ty
          ...
          let d = Cesium.Cartesian3.distance(from, to);
          let t = d / uvw[3];
  ...
  _randomParticle (particle) {
    ...
    var nextLng = lng + this.calc_speedRate[0] * uvw[0]
    var nextLat = lat + this.calc_speedRate[1] * uvw[1]
    particle.lng = lng
    particle.lat = lat
    //计算随机点的高程 (mock height data)
...
function CanvasHeightParticle () {
  ...
  this.height = 0// 粒子当前高度
  this.theight = 0// 粒子下一步高度
}