...
// 水体渲染器类
class WaterRenderer {
  constructor(viewer, width, height, waterLevel = 0) {
    this._viewer = viewer;
    this._width = width;
    this._height = height;
  ...
  createWaterPrimitive () {
    try {
      console.log("开始创建水体primitive...");
      console.log("Cesium.Geometry 可用:", !!Cesium.Geometry);
      console.log("Cesium.GeometryAttribute 可用:", !!Cesium.GeometryAttribute);

      // 创建水体几何体 - 覆盖整个地形区域
      const positions = new Float32Array([
        -1.0, -1.0, this._waterLevel,
        1.0, -1.0, this._waterLevel,
        1.0, 1.0, this._waterLevel,
        -1.0, 1.0, this._waterLevel
      ]);
      ...
      void main() {
        ...
        // 添加波浪动画
        float wave1 = sin(position.x * 10.0 + u_time * 2.0) * 0.02;
        float wave2 = sin(position.y * 8.0 + u_time * 1.5) * 0.015;
        float wave3 = sin(position.x * 15.0 + position.y * 12.0 + u_time * 3.0) * 0.01;
        
        vec3 animatedPosition = position;
        animatedPosition.z += wave1 + wave2 + wave3;
        
        gl_Position = czm_modelViewProjection * vec4(animatedPosition, 1.0);
      }
    `;
      ...
      void main() {
        ...
        // 添加波浪纹理
        float wave1 = sin(v_st.x * 20.0 + u_time * 2.0) * 0.5 + 0.5;
        float wave2 = sin(v_st.y * 15.0 + u_time * 1.5) * 0.5 + 0.5;
        float wave3 = sin(v_st.x * 30.0 + v_st.y * 25.0 + u_time * 3.0) * 0.5 + 0.5;
        
        // 组合波浪效果
        float wavePattern = (wave1 + wave2 + wave3) / 3.0;
        
        // 添加泡沫效果
        float foam = smoothstep(0.6, 0.8, wavePattern);
        ...
        // 添加深度变化
        float depth = 1.0 - v_st.y;
        finalColor = mix(finalColor, waterColor * 0.7, depth * 0.3);
    ...
    `;
      ...
      // 创建水体primitive
      const waterPrimitive = new CustomPrimitive({
        commandType: 'Draw',
        geometry: geometry,
        attributeLocations: {
          position: 0,
        ...
        vertexShaderSource: new Cesium.ShaderSource({
          sources: [waterVertexShader]
        }),
        fragmentShaderSource: new Cesium.ShaderSource({
          sources: [waterFragmentShader]
        })
      });

      this._waterPrimitive = waterPrimitive;
      // 水体在地形之后渲染，所以后添加
      this._viewer.scene.primitives.add(waterPrimitive);
    } catch (error) {
      console.error("创建水体primitive失败:", error);
    }
  }
  ...
  setWaterLevel (level) {
    ...
    // 重新创建primitive以更新高度
    this._viewer.scene.primitives.remove(this._waterPrimitive);
    this.createWaterPrimitive();
  }
}
...
const readGeoTif = async () => {
  try {
    ...
    if (typeof Cesium !== 'undefined' && Cesium.Geometry) {
      // 创建水体渲染器，设置水体高度为50米以覆盖大部分地形
      const waterRenderer = new WaterRenderer(viewer, width, height, 50);

      // 添加键盘控制水体高度
      window.waterRenderer = waterRenderer; // 全局访问以便调试
      console.log("水体渲染器已创建，使用以下命令调整水体高度：");
      console.log("waterRenderer.setWaterLevel(高度值)");
      console.log("例如：waterRenderer.setWaterLevel(100)");
    } else {
      console.error("Cesium 对象不可用，无法创建水体渲染器");
    }
  } catch (e) {
