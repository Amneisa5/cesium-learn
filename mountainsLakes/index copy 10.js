...
vec2 getHeight(in vec3 p)
{
 ...
 // h.x 是地形高度，h.y 是水位
 // 总高度 = 地形高度 + 水位，但水位不能超过地形高度
 float terrainHeight = h.x;
 float waterLevel = h.y;
 
 // 确保水位不会超过地形高度
 // 在海底区域，水位应该填充到海平面（0.5）
 float seaLevel = 0.5; // 海平面在归一化坐标系中的位置
 float maxWaterLevel = max(0.0, seaLevel - terrainHeight);
 float actualWaterLevel = min(waterLevel, maxWaterLevel);
 float totalHeight = terrainHeight + actualWaterLevel;
...
vec3 terrainColor(in vec3 p, in vec3 n, out float spec)
{
 ...
 // 检查是否为海底区域（使用纹理的第三个通道）
 vec4 heightData = texture(iChannel0, (p.xz + 0.5));
 ...
 if (isUnderwater) {
   // 海底区域：深蓝色渐变
   float depth = 0.5 - relativeHeight; // 深度
   vec3 deepBlue = vec3(0.0, 0.1, 0.3); // 深蓝
   vec3 midBlue = vec3(0.0, 0.3, 0.6);  // 中蓝
   vec3 lightBlue = vec3(0.2, 0.5, 0.8); // 浅蓝
   
   ramp = mix(deepBlue, midBlue, smoothstep(0.0, 0.3, depth));
   ramp = mix(ramp, lightBlue, smoothstep(0.3, 0.6, depth));
   spec = 0.8; // 水的高反射
 } else {
   // 陆地：原有的颜色方案
   ...
   ramp = mix(c1, c2, smoothstep(0.05, 0.35, nh));
...
vec3 Render(in vec3 ro, in vec3 rd) {
 ...
 if(ret.x > 0.0) {
  ...
  if(wt < ret.y) {
    ...
    if (isUnderwater) {
      // 海底区域：增强水流效果
      float waterDepth = 0.5 - relativeHeight;
      vec3 waterColor = mix(vec3(0.0, 0.2, 0.4), vec3(0.0, 0.4, 0.8), smoothstep(0.0, 0.5, waterDepth));
      tc = mix(tc, waterColor, 0.7); // 混合水色
      
      // 增强水波效果
      float wave = sin((ro + rd * wt).x * 10.0 + iTime * 2.0) * 0.1;
      wave += sin((ro + rd * wt).z * 8.0 + iTime * 1.5) * 0.05;
      tc += wave * vec3(0.1, 0.2, 0.3);
    } else {
      // 陆地：原有的雾效果
...
const readGeoTif = async () => {
  ...
  // 以零点为基准，调整高度范围
  const seaLevel = 0; // 海平面为零点
  const heightRange = Math.max(maxHeight - seaLevel, seaLevel - minHeight);
  const adjustedMinHeight = seaLevel - heightRange;
  const adjustedMaxHeight = seaLevel + heightRange;

  console.log("原始高度范围:", minHeight, "到", maxHeight, "米");
  console.log("调整后高度范围:", adjustedMinHeight, "到", adjustedMaxHeight, "米");
  console.log("海平面零点:", seaLevel, "米");
  ...
  for (let y = 0; y < tifHeight; y++) {
    for (let x = 0; x < tifWidth; x++) {
      ...
      // 以零点为基准进行归一化
      const normalizedHeight = (rawHeight - adjustedMinHeight) / (adjustedMaxHeight - adjustedMinHeight);

      // 计算相对于海平面的高度（0.5表示海平面）
      const relativeHeight = (rawHeight - seaLevel) / heightRange + 0.5;

      // 设置初始水位：只在海底区域有水位
      let initialWater = 0;
      if (rawHeight < seaLevel) {
        // 海底区域：水位等于海平面与海底的差值
        const waterDepth = seaLevel - rawHeight;
        // 将水位归一化，但限制在合理范围内
        // 0.5 表示海平面，所以水位应该是 (0.5 - 地形高度)
        const normalizedTerrainHeight = normalizedHeight; // 已经归一化的地形高度
        const seaLevelNormalized = 0.5; // 海平面在归一化坐标系中的位置
        initialWater = Math.max(0, seaLevelNormalized - normalizedTerrainHeight);
        initialWater = Math.min(initialWater, 0.2); // 限制最大水位
      }
