...
vec3 terrainColor(in vec3 p, in vec3 n, out float spec)
{
 ...
 // 如果是海底区域，轻微混合水色，但保持地形特征
 if (isUnderwater) {
   float depth = 0.5 - relativeHeight; // 深度
   vec3 waterTint = mix(vec3(0.0, 0.2, 0.4), vec3(0.0, 0.4, 0.8), smoothstep(0.0, 0.5, depth));
   ramp = mix(ramp, waterTint, 0.2); // 只混合20%的水色，保持80%的地形特征
   spec = mix(spec, 0.8, 0.3); // 部分水反射
   
   // 增强地形对比度，让地形特征更明显
   ramp = mix(ramp, ramp * 1.2, 0.5); // 提高对比度
 }
 ...
 if(ret.x > 0.0) {
  ...
  if(wt < ret.y) {
    ...
    if (isUnderwater) {
      ...
      tc = mix(tc, waterColor, 0.3); // 减少水色混合到30%
      
      // 轻微的水波效果
      float wave = sin((ro + rd * wt).x * 10.0 + iTime * 2.0) * 0.05;
      wave += sin((ro + rd * wt).z * 8.0 + iTime * 1.5) * 0.03;
