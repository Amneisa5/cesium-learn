Cesium.Ion.defaultAccessToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiJjOTBiZjBiYy1jMzhjLTQ1ZDQtOWQyYy1kOTU4MzUyZWU4NDYiLCJpZCI6MTc1MDE3LCJpYXQiOjE3NjA5MjkyMjB9.FelaYKogjfjIDczLRFmPNvsugZZO1IeMRv2FnkRnXKE'
...
async function loadTileset (resource, modelMatrix) {
  ...
  tilesetClippingPlanes = new Cesium.ClippingPlaneCollection({
    planes: [
  ...
  try {
    ...
    tileset = await Cesium.Cesium3DTileset.fromUrl(url, {
      clippingPlanes: tilesetClippingPlanes,
    });
    ...
    for (let i = 0; i < tilesetClippingPlanes.length; ++i) {
      const plane = tilesetClippingPlanes.get(i);
...
function loadModel (url) {
  modelClippingPlanes = new Cesium.ClippingPlaneCollection({
    planes: [
      new Cesium.ClippingPlane(new Cesium.Cartesian3(0.0, 0.0, -1.0), 0.0),
    ],
    edgeWidth: viewModel.edgeStylingEnabled ? 1.0 : 0.0,
  ...
  for (let i = 0; i < modelClippingPlanes.length; ++i) {
    const plane = modelClippingPlanes.get(i);
    const planeEntity = viewer.entities.add({
      position: position,
      plane: {
...
Cesium.knockout
  ...
  .subscribe(function (value) {
    ...
    if (Cesium.defined(tilesetClippingPlanes)) {
      tilesetClippingPlanes.edgeWidth = edgeWidth;
    }
    if (Cesium.defined(modelClippingPlanes)) {
      modelClippingPlanes.edgeWidth = edgeWidth;
    }
  });
...
function reset () {
  ...
  targetY = 0.0;
  tileset = undefined;
  tilesetClippingPlanes = undefined;
