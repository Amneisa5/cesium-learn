...
vec2 readHeight(ivec2 p){
  ...
  float terrainH = texelFetch(uDemTexture, p, 0).r;
...
void main( ){
  ...
  // Terrain
  vec2 uv = gl_FragCoord.xy / vec2(float(terrainWidth), float(terrainHeight));
  float t = iTime / transitionTime;
  float h = texture(uDemTexture, uv).r ;
  ...
  vec2 texUV = vec2(uv.x, 1.0 - uv.y); // 👈 关键：翻转 Y 轴
  float terrainElevation = texture(heightMap, texUV).r;
...
const renderShaderSource = `
  ...
  vec2 getHeight(in vec3 p){
    p = (p + 1.0) * 0.5;
    vec2 p2 = p.xz * vec2(float(terrainWidth), float(terrainHeight)) / iResolution.xy;
    p2 = min(p2, vec2(float(terrainWidth) - 0.5, float(terrainHeight) - 0.5) / iResolution.xy);
...
class FluidDemo {
  ...
  _height;
  _resolution;
  constructor(viewer, width, height, terrainData) {
    this._viewer = viewer;
    this._terrainData = terrainData;
    console.log(this._terrainData)
    // 分辨率
    this._width = width;
    this._height = height;
  ...
  initShaderToy () {
    const texA = RenderUtil.createTexture({
      ...
      arrayBufferView: this._terrainData,
    });
    ...
    const texture = new Cesium.Texture({
      ...
      source: {
        arrayBufferView: this._terrainData,  // Float32Array
        width: 840,
        height: 600
      },
    ...
    const heightTexture = new Cesium.Texture({
      ...
      width: this._width,
      height: this._height,
      pixelFormat: Cesium.PixelFormat.R32F, // 单通道浮点
      pixelDatatype: Cesium.PixelDatatype.FLOAT,
      source: {
        arrayBufferView: this._terrainData, // 注意：这里只传 R 通道，所以 textureData 应只含高度
      },
...
const readGeoTif = async () => {
  ...
  // 添加流体
  const fluid = new FluidDemo(viewer, width, height, textureData);
}
