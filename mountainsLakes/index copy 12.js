...
vec3 Render(in vec3 ro, in vec3 rd) {
 ...
 if(ret.x > 0.0) {
  ...
  bool isUnderwater = terrainHeight < seaLevelHeight;
  ...
  if (isUnderwater && wt < ret.y) {
    ...
    // 海底区域：非常轻微的水效果，保持地形特征
    float waterDepth = seaLevelHeight - terrainHeight;
    vec3 waterColor = mix(vec3(0.0, 0.2, 0.4), vec3(0.0, 0.4, 0.8), smoothstep(0.0, 0.5, waterDepth));
    tc = mix(tc, waterColor, 0.15); // 进一步减少水色混合到15%
  ...
  for (let y = 0; y < tifHeight; y++) {
    for (let x = 0; x < tifWidth; x++) {
      ...
      if (minHeight > 0) {
        ...
        normalizedHeight = (rawHeight - minHeight) / (maxHeight - minHeight);
        relativeHeight = (rawHeight - seaLevel) / (2 * maxRange) + 0.5;

        // 为了着色器使用，我们需要将海平面映射到0.5
        // 所以normalizedHeight应该基于海平面进行归一化
      ...
      if (minHeight > 0) {
        ...
        initialWater = waterDepth / maxRange;
        initialWater = Math.min(initialWater, 0.2);
      } else {
        // 数据跨越海平面
        if (rawHeight < seaLevel) {
          const waterDepth = seaLevel - rawHeight;
          initialWater = waterDepth / (2 * maxRange);
          initialWater = Math.min(initialWater, 0.2);
        }
      }
